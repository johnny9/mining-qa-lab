...
- `PYTHONPATH=src python -m unittest discover -s tests/unit -v` passed all 69
  unit, web/API, skill, and spec tests in a Python 3.13 container with test
  extras on 2026-08-16.
...
- AC-02, AC-04, AC-05, and AC-11 remain unchecked pending service-loop
  registration, integration with the full inventory/preflight/resource lease
  engine, retry-attempt support, operator controls, and production backoff/
  retention evidence.
