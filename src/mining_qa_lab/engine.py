...
MAX_RESULT_POINTER_BYTES = 64 * 1024


def _load_result_pointer(path: Path) -> dict[str, Any]:
    if not path.exists():
        raise PublishError("runner did not write a result pointer")
    with path.open("rb") as stream:
        raw = stream.read(MAX_RESULT_POINTER_BYTES + 1)
    if len(raw) > MAX_RESULT_POINTER_BYTES:
        raise PublishError(
            f"result pointer exceeds {MAX_RESULT_POINTER_BYTES} bytes"
        )
    try:
        payload = json.loads(raw.decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise PublishError(f"invalid result pointer: {exc}") from exc
    if not isinstance(payload, dict):
        raise PublishError("result pointer must be a JSON object")
    contract_version = payload.get("contract_version", ORCHESTRATION_CONTRACT_VERSION)
    if (
        isinstance(contract_version, bool)
        or not isinstance(contract_version, int)
        or contract_version != ORCHESTRATION_CONTRACT_VERSION
    ):
        raise PublishError(
            f"unsupported result-pointer contract version: {contract_version!r}"
        )
    publishers = payload.get("publishers", [])
    if not isinstance(publishers, list) or not all(
        isinstance(publisher, dict) for publisher in publishers
    ):
        raise PublishError("result pointer publishers must be a list of objects")
    status = payload.get("status")
    if status not in {"passed", "failed", "error", "skipped"}:
        raise PublishError(f"invalid result pointer status: {status!r}")
    for publisher in publishers:
        if (
            not isinstance(publisher.get("name"), str)
            or not isinstance(publisher.get("success"), bool)
            or not isinstance(publisher.get("required"), bool)
        ):
            raise PublishError(
                "result pointer publisher requires name, success, and required"
            )
    return payload
...
class AssignmentExecutor:
    ...
    def execute(self, assignment: Mapping[str, Any]) -> None:
        ...
        if host["transport"] == "ssh":
            remote_root = Path(str(host.get("work_root") or "/tmp/mining-qa-lab"))
        ...
        try:
            if host["transport"] == "local":
                ...
                output = result.stdout
                returncode = result.returncode
                pointer_command = shlex.join(
                    [
                        "head",
                        "-c",
                        str(MAX_RESULT_POINTER_BYTES + 1),
                        "--",
                        str(result_pointer),
                    ]
                )
